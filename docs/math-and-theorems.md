...
- **The `0.4` round-off threshold is a tuned conservative bound, not a theorem constant.** It's below the provable safe limit with margin; treat it as an engineering parameter justified by the multiplication theory, not a magic number (`arithmetic-foundation.md` §8).
