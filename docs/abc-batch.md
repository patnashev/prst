...
- **`File::get_textreader` / `read_buffer`.** The line-reading path is the `File` abstraction's text mode; covered (as on-disk I/O) by the future `state-serialization.md`. Here it's just "read the lines."
