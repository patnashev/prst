...
int net_main(int argc, char *argv[])
{
	...
	while (true)
	{
        ...
        {
            if (net.task()->hex)
                input.parse("Hex(" + (net.task()->sk == "1" ? "" : net.task()->sk == "-1" ? "-" : net.task()->sk + "*") + (net.task()->sb == "!" || net.task()->sb == "#" ? std::to_string(net.task()->n) + net.task()->sb : net.task()->sb + "^" + std::to_string(net.task()->n)) + ")");
            else if (net.task()->cyclotomic != 0)
                input.parse("Phi(" + std::to_string(net.task()->cyclotomic) + "," + (net.task()->sk == "1" ? "" : net.task()->sk == "-1" ? "-" : net.task()->sk + "*") + (net.task()->sb == "!" || net.task()->sb == "#" ? std::to_string(net.task()->n) + net.task()->sb : net.task()->sb + "^" + std::to_string(net.task()->n)) + ")");
